...
class Acp35Fan(IntEnum):    LOW = 1; MEDIUM = 2; HIGH = 3   # no member for 0
...
| ~~12~~ | ~~Does the unit accept fan `0` (`b6` = `0x0x`) as an auto speed?~~ **Closed 2026-08-19 by observation, not by test.** The appliance and the remote both have exactly three fan speeds, so there is no fourth for `0` to select. `Acp35Fan` has no member for it: a name invented from the nibble's width is how a guess reaches a user, since an editor offers it beside the real speeds. A frame carrying `0` is refused by the decoder, like the twelve other values each nibble of `b6` can hold and this protocol has no meaning for | — | — |
